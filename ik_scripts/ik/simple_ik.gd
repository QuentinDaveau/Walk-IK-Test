...
func _setup_joints() -> void:
	...
	var first_joint := get_node(_joints[0].target_transform)
	_joints_resolvers.push_front(JointResolver.new(first_joint, 0.0, 0.01, null, ForwardLocker.new(first_joint.get_parent(), first_joint.transform.basis.z)))
...
class JointResolver:
	var _joint: Spatial
	var _magnet: Spatial
	var _forward_locker: ForwardLocker
	...
	func _init(joint: Spatial, distance: float, constraint: float, magnet: Spatial = null, forward_locker: ForwardLocker = null) -> void:
		_joint = joint
		bone_distance = distance
		_constraint = constraint
		_magnet = magnet
		_forward_locker = forward_locker
		refresh_position()
	...
	func refresh_position() -> void:
		...
		if _forward_locker:
			forward = -_forward_locker.get_global_forward()
		else:
	...
	func resolve_constraint(source_position: Vector3) -> Vector3:
		...
		if not _constraint or not direction or not forward:
			return direction
		else:
...
class ForwardLocker:
	...
	func _init(target: Spatial, forward: Vector3) -> void:
		_target = target
		_forward = forward
	
	
	func get_global_forward() -> Vector3:
		return _target.global_transform.basis.xform(_forward)
